...
import net.ehicks.cinemang.SystemInfo;
...
public class DBMap
{
    ...
    public static String pathToBeans = "/WEB-INF/classes/net/ehicks/cinemang/beans";
    public static String packageBase = "net.ehicks.cinemang.beans.";
    ...
    public static void loadDbMaps()
    {
        try
        {
            File beans = Paths.get(SystemInfo.getServletContext().getRealPath(pathToBeans)).toFile();
            for (String bean : beans.list())
            {
                if (new File(SystemInfo.getServletContext().getRealPath(pathToBeans + File.separator + bean)).isFile())
                {
                    DBMap dbMap = new DBMap();

                    Class beanClass = Class.forName(packageBase + bean.substring(0, bean.lastIndexOf(".")));
